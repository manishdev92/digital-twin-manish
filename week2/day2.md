...
Update `frontend/components/twin.tsx` — the `fetch` call must use your **API Gateway invoke URL** from **Part 6 → Step 6** (the one that looks like `https://abc123.execute-api.us-east-1.amazonaws.com`), plus the path **`/chat`**.

**Do not confuse this with your S3 website URL.** Part 7 Step 5 and Part 8 give you an `http://...s3-website...amazonaws.com` (or CloudFront) address — that is only where you **open the app in the browser**. If you paste the S3 website URL into `fetch`, the browser will not hit Lambda; you will get HTML or errors instead of JSON, and the UI will show a generic “try again” message.

```typescript
// Local dev:
// const response = await fetch('http://localhost:8000/chat', {

// Production: API Gateway (HTTPS, execute-api host — NOT the S3 website host)
const chatApiUrl =
...
Replace `YOUR-API-ID` and `us-east-1` with your real API id and region from the console. Optionally set `NEXT_PUBLIC_CHAT_API_URL` when running `npm run build` so you do not have to edit the file for each environment.
...
### Step 6: Allow your S3 website origin in API Gateway CORS (required for chat)

Browsers send a **preflight** `OPTIONS` request before `POST /chat` (because of `Content-Type: application/json`). If API Gateway only allows `http://localhost:3000` and **not** your static site origin, preflight returns **400** with `Disallowed CORS origin` and the UI shows a generic “try again” error — while `curl` to the same API still works.

1. In **API Gateway** → your HTTP API → **CORS** → **Configure**
2. Under **Access-Control-Allow-Origin**, **Add** your exact bucket website origin (no trailing slash), for example:  
   `http://YOUR-BUCKET.s3-website.REGION.amazonaws.com`  
   Keep `http://localhost:3000` as well if you still develop locally.
3. Alternatively set the origin to `*` **and** ensure you clicked **Add** so it is saved (see Part 6 Step 5).
4. Click **Save**, wait a minute, then hard-refresh the static site and try chat again.

Quick check from a terminal (replace URL and region):

```bash
curl -i -X OPTIONS 'https://YOUR-API-ID.execute-api.REGION.amazonaws.com/chat' \
  -H 'Origin: http://YOUR-BUCKET.s3-website.REGION.amazonaws.com' \
  -H 'Access-Control-Request-Method: POST' \
  -H 'Access-Control-Request-Headers: content-type'
```

You want **`HTTP/2 200`** (or `204`), not **400**.

**If you still get `400` and `Disallowed CORS origin` after API Gateway looks correct:** your HTTP API likely routes **`OPTIONS`** to **Lambda** (e.g. `OPTIONS` → `/{proxy+}`). Then **FastAPI** handles preflight and only allows origins listed in the **`CORS_ORIGINS` environment variable** on the function — not only API Gateway’s CORS screen.

1. Open **Lambda** → your function (e.g. `twin-api`) → **Configuration** → **Environment variables**
2. Edit **`CORS_ORIGINS`** to include **every** frontend origin, comma-separated, **no spaces** (or spaces are OK with a recent `server.py` that trims), for example:  
   `http://localhost:3000,http://YOUR-BUCKET.s3-website.REGION.amazonaws.com`  
   For a quick course setup you can use `*` (wildcard) if you are comfortable with it.
3. **Save**, wait for the update, then run the same `curl -X OPTIONS` test again until you get **200**.

Redeploying the Lambda package is **not** required for env-only changes.
...
1. **API Gateway + Lambda CORS:** Every **origin** you open the app from must be allowed for **preflight**. Configure **both** API Gateway **CORS** and Lambda **`CORS_ORIGINS`** (see Part 7 Step 6). If `curl -X OPTIONS` still returns **400 Disallowed CORS origin** after API Gateway is updated, **`OPTIONS` is hitting Lambda** — fix **`CORS_ORIGINS`** on the function, not only API Gateway.
2. Verify Lambda environment variable `CORS_ORIGINS` includes your CloudFront URL with "https://" at the start and no trailing "/" - THIS MUST BE PRECISELY RIGHT!
3. Check API Gateway CORS configuration
4. Make sure OPTIONS route is configured
